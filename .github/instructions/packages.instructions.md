...
applyTo: "packages/**,stacks/unicas/**"
---

# UniCAS package boundaries

UniCAS is an independently deployable middleware. Keep `packages/` free of runtime dependencies on `@unidocs/*` packages.

## Actor-facing client packages

- Keep the admin and Space data access planes separate. Actor-facing package names use the `admin-` and `space-` prefixes respectively.
...
- Put protocol types shared by both access planes in `@unicas/space-protocol`. `@unicas/admin-protocol` may depend on `@unicas/space-protocol`; the reverse dependency is forbidden.
...
- Higher-level packages may wrap a client to reduce caller complexity. Keep that business abstraction separate from the transport client; `@unicas/space-blob-client` wrapping `@unicas/space-client` is the reference pattern.
