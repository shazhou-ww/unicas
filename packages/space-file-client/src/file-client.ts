...
import { FileManifestContentType, type SpaceFileManifestEntry } from "./file-protocol.js";
import type {
  SpaceFileRoot,
  SpaceFileRootInfo,
  SpaceFileStat,
...
export function createSpaceFileSystem(options: SpaceFileSystemOptions): SpaceFileSystem {
  ...
  async function storeManifest(entries: ReadonlyMap<string, WorkingEntry>): Promise<string> {
    ...
    const refs: string[] = [];
    const manifestEntries: SpaceFileManifestEntry[] = sorted.map(([path, entry]) => {
  ...
  function workingRoot(initialInfo: SpaceFileRootInfo, initialEntries: ReadonlyMap<string, WorkingEntry>): SpaceFileRoot {
...
function toStat(path: string, entry: WorkingEntry): SpaceFileStat {
...
function compareStats(left: SpaceFileStat, right: SpaceFileStat): number {
