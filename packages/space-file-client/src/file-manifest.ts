...
import {
  ...
  FileManifestVersion,
  type SpaceFileEntry,
  type SpaceFileManifestEntry,
  type SpaceFileManifestV1,
} from "./file-protocol.js";

type WireEntry = readonly ["d", string] | readonly ["f", string, number, number, string];

export function validateFileManifest(value: SpaceFileManifestV1): void {
...
export function createFileManifest(entries: readonly SpaceFileManifestEntry[]): SpaceFileManifestV1 {
  const sorted = entries.map((entry) => ({ ...entry })).sort((left, right) => compareFileManifestPaths(left.path, right.path));
  let ref = 0;
  const normalized = sorted.map((entry): SpaceFileManifestEntry =>
...
export function encodeFileManifest(value: SpaceFileManifestV1): Uint8Array {
...
export function decodeFileManifest(bytes: Uint8Array): SpaceFileManifestV1 {
  ...
  const manifest = { version: decoded.v, entries } as SpaceFileManifestV1;
  validateFileManifest(manifest);
  if (!equalBytes(bytes, encodeFileManifest(manifest))) throw new TypeError("File manifest encoding is not canonical");
  return freezeManifest(manifest);
}

export function fileManifestRefs(manifest: SpaceFileManifestV1, refs: readonly string[]): ReadonlyMap<string, string> {
  validateFileManifest(manifest);
  const files = manifest.entries.filter((entry): entry is SpaceFileEntry => entry.type === "file");
...
function decodeEntry(value: unknown): SpaceFileManifestEntry {
...
function freezeManifest(value: SpaceFileManifestV1): SpaceFileManifestV1 {
