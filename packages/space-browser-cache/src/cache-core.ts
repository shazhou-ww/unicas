...
import type { CasHash, CasNodeMetadata, CasNodeRange } from "@unicas/space-client";

export interface BrowserNodeCacheCoreOptions<Key extends { readonly hash: CasHash }> {
  readonly namespace: { readonly endpoint: string; readonly principal: string };
  readonly databaseName: string;
  readonly maxBytes?: number;
  readonly maxMemoryBytes?: number;
  readonly maxEntryBytes?: number;
  readonly namespaceParts: (endpoint: string, principal: string) => readonly unknown[];
  readonly keyParts: (key: Key, kind: "metadata" | "content") => readonly unknown[];
}

export interface BrowserNodeCacheCore<Key extends { readonly hash: CasHash }> {
  metadata(
    key: Key,
...
export async function clearBrowserNodeCachesCore(options: {
...
export function createBrowserNodeCacheCore<Key extends { readonly hash: CasHash }>(
  options: BrowserNodeCacheCoreOptions<Key>,
): BrowserNodeCacheCore<Key> {
  ...
  const endpointValue = endpoint.href.replace(/\/$/, "");
  const namespace = JSON.stringify(options.namespaceParts(endpointValue, principal));
  ...
  function keyId(key: Key, kind: "metadata" | "content"): string {
    return JSON.stringify([namespace, ...options.keyParts(key, kind)]);
  }
  ...
  const cache: BrowserNodeCacheCore<Key> = {
